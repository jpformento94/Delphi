...
Begin
  ...
  if (alSize >= MinTable) And (alSize <= MaxTable) then Begin
    ...
    End;
    for I := MinTable To (alSize div 2) Do Begin
      ZO[I].Ocupado := False;
...
Begin
  ...
  Else Begin
    ...
    Else Begin
      ...
      Q := MinTable;
      Encontre := False;
      While (Q <= (Size Div 2)) And (Not(Encontre)) Do Begin
...
// Busco y Elimina una Clave de la ZO - Funcion Interna del Objeto
...
Begin
  ...
  Encontre := False;
  While (Q <= (Size Div 2)) And (Not(Encontre)) Do Begin
    If ZO[Q].Ocupado = True Then Begin
      If X.Clave = ZO[Q].Clave.Clave  Then
        Encontre := True
      Else
        Inc(Q);
...
Begin
  ...
  // Ahora Recorro la ZO
  For I := MinTable To (Size Div 2) Do Begin
    If ZO[I].Ocupado = True Then Begin
      X := ZO[I].Clave;
      S := X.ArmarString;
      SS:= SS + S + cCRLF;
    End;
